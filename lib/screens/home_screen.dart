...
enum ProductSortOption { latest, alphabetical }
...
class _HomeScreenState extends State<HomeScreen>
  ...
  final _sortOptionNotifier = ValueNotifier<ProductSortOption>(
    ProductSortOption.latest,
  );
  ...
  ProductSortOption get _sortOption => _sortOptionNotifier.value;

  // Cache para resultados filtrados
  List<Map<String, dynamic>>? _cachedFilteredProducts;
  String _lastSearchQuery = '';
  String _lastSelectedCategory = '';
  ProductSortOption _lastSortOption = ProductSortOption.latest;
  ...
  void initState() {
    ...
    _sortOptionNotifier.addListener(_onCategoryChanged);

    // Carregar favoritos e escutar mudanças
    _favoritesService.addListener(_onFavoritesChanged);
    _favoritesService.loadFavorites();
    _catalogSyncListener = () {
      _scheduleCatalogReload(preserveScrollPosition: true);
    };
      ...
      _productsChannel = supabase
          ...
          .onPostgresChanges(
            ...
            callback: (payload) {
              ...
              debugPrint('📥 Agendando recarga após atualização em tempo real');
              _scheduleCatalogReload(preserveScrollPosition: true);
            },
          )
          .subscribe();
  ...
  void _scheduleCatalogReload({bool preserveScrollPosition = true}) {
    _catalogRefreshDebounce?.cancel();
    _catalogRefreshDebounce = Timer(const Duration(milliseconds: 250), () {
      if (!mounted) return;
      _loadData(preserveScrollPosition: preserveScrollPosition);
    });
  }
  ...
  List<Map<String, dynamic>> get _filteredProducts {
    ...
    if (_cachedFilteredProducts == null ||
      ...
      _cachedFilteredProducts =
          List<Map<String, dynamic>>.from(_cachedFilteredProducts!)..sort((
            a,
            b,
          ) {
            final availabilityCompare = (a['ativo'] == false ? 1 : 0).compareTo(
              b['ativo'] == false ? 1 : 0,
            );
            ...
            if (_sortOption == ProductSortOption.alphabetical) {
  ...
  List<Widget> _buildSpecialSections() {
    ...
    final cardWidth = _specialSectionCardWidth(context);
    final sectionHeight = _specialSectionHeight(context);
  ...
  double _specialSectionCardWidth(BuildContext context) {
    final screenWidth = MediaQuery.of(context).size.width;
    return (screenWidth * 0.46).clamp(172.0, 220.0).toDouble();
  }

  double _specialSectionHeight(BuildContext context) {
    final screenWidth = MediaQuery.of(context).size.width;
    final cardWidth = _specialSectionCardWidth(context);
    final height = cardWidth * (screenWidth < 380 ? 1.62 : 1.46);
    return height.clamp(286.0, 308.0).toDouble();
  }
      ...
      child: _isLoading
          ...
          : NotificationListener<ScrollNotification>(
              onNotification: (ScrollNotification scrollInfo) {
                ...
                if (scrollInfo.metrics.pixels > 50 && !_hasScrolledDown) {
                  _hasScrolledDown = true;
                }
                if (scrollInfo.metrics.pixels > 50 && _showScrollIndicator) {
                  _hideScrollIndicator();
                }
                      ...
                      RefreshIndicator(
                        ...
                        child: CustomScrollView(
                          ...
                          slivers: [
                            ...
                            SliverToBoxAdapter(
                              child: ValueListenableBuilder<ProductSortOption>(
          ...
          child: Card(
            ...
            child: LayoutBuilder(
              builder: (context, constraints) {
                final isCompactCard = constraints.maxWidth < 190;
                final imageHeight = isCompactCard ? 106.0 : 116.0;
                final contentPadding = isCompactCard ? 8.0 : 9.0;
                final titleFontSize = isCompactCard ? 12.5 : 13.0;
                final descriptionFontSize = isCompactCard ? 9.5 : 10.0;
                final buttonHeight = isCompactCard ? 34.0 : 36.0;
                final iconSize = isCompactCard ? 13.0 : 14.0;
                final buttonTextSize = isCompactCard ? 10.0 : 11.0;
                final textBlockHeight = isCompactCard ? 56.0 : 62.0;
                final contentToPriceSpacing = isCompactCard ? 6.0 : 8.0;
                final priceToButtonsSpacing = isCompactCard ? 8.0 : 10.0;
                        ...
                        Positioned(
                          ...
                          child: GestureDetector(
                            onTap: () async {
                              ...
                              try {
                                debugPrint(
                                  '🔊 Tentando reproduzir som de feedback...',
                                );
                                if (wasFavorite) {
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            SizedBox(height: contentToPriceSpacing),
                            _buildProductPrice(produto, compact: isCompactCard),
                            SizedBox(height: priceToButtonsSpacing),
                            // Botões
                            Row(
                              children: [
  ...
  Widget _buildProductPrice(
    ...
    final priceInfo = _getProductDisplayPriceInfo(produto);
    ...
    final currentPriceStyle = TextStyle(
      fontSize: compact ? 15 : 16,
      fontWeight: FontWeight.bold,
      color: hasDiscount ? Colors.orange : Colors.green,
      height: 1.1,
    );
    final priceBlockHeight = compact ? 52.0 : 56.0;
      ...
      child: Column(
        ...
        children: [
          hasDiscount && previousPrice != null
              ...
              : Text(
                  ' ',
                  style: oldPriceStyle.copyWith(color: Colors.transparent),
                ),
          const SizedBox(height: 2),
          Text(hasMultiplePrices ? 'A partir de' : 'Preço', style: labelStyle),
          ...
          Wrap(
            ...
            children: [
              ...
              if (discountPercentage != null) ...[
                _buildDiscountChip(discountPercentage, compact: compact),
              ],
            ],
          ),
        ],
  ...
  Widget _buildDiscountChip(int discountPercentage, {bool compact = false}) {
    return Container(
      padding: EdgeInsets.symmetric(
        horizontal: compact ? 8 : 10,
        vertical: compact ? 3 : 4,
      ),
      constraints: BoxConstraints(minWidth: compact ? 54 : 60),
      decoration: BoxDecoration(
        color: Colors.orange,
        borderRadius: BorderRadius.circular(12),
        boxShadow: [
